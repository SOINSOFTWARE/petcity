...
if (isset($_POST['save'])) {
	...
	if (intval($id) === 0) {
		...
		if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $filepath)) {
			$saved = $mdexamtable -> insert($idconsultation, $examdateToSQL, $name, $results, $filepath, $formulanumber, $formula);
			if ($saved === TRUE) {
				$id = $mdexamtable -> selectLastInsertId();
			}
		} else {
			$errormovingfile = TRUE;
		}
	} else {
		$saved = $mdexamtable -> update($id, $examdateToSQL, $name, $results, $formulanumber, $formula);
	}
	if ($saved === FALSE) {
		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="right-side">
				...
				<section class="content">
					...
					<div class="row">
						<form action="examen.php" method="post" role="form" onsubmit="return validate()" enctype="multipart/form-data">
							<div class="col-xs-12">
								<div class="box">
									<?php
									if (isset($saved)) {
