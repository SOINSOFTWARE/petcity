...
if (isset($_POST['save'])) {
	...
	if (intval($id) === 0) {
		...
		if ($generaldatasaved === TRUE) {
			...
			if ($nextdate != '') {
				$saved = $surgerycontroltable -> insert($idsurgery, $idgeneraldata, $evolution, $diagnosisrecomendations, $diagnosissamples, $diagnosisexams, $nextdateToSQL);
			} else {
				$saved = $surgerycontroltable -> insertNonNextDate($idsurgery, $idgeneraldata, $evolution, $diagnosisrecomendations, $diagnosissamples, $diagnosisexams);
			}
			if ($saved === TRUE) {
				$id = $surgerycontroltable -> selectLastInsertId();
			}
		} else {
		...
		if ($generaldatasaved === TRUE) {
			if ($nextdate != '') {
				$saved = $surgerycontroltable -> update($id, $evolution, $diagnosisrecomendations, $diagnosissamples, $diagnosisexams, $nextdateToSQL);
			} else {
				$saved = $surgerycontroltable -> updateNonNextDate($id, $evolution, $diagnosisrecomendations, $diagnosissamples, $diagnosisexams);
			}
		} else {
			$errorLog = new ErrorLogTable();
			$errorLog -> insert($generalDataTable -> getError());
...
if (isset($_POST['view'])) {
	$id = $_POST['idsurgerycontrol'];
	$results = $surgerycontroltable -> selectById($id);
...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Pet City | Control post-procedimiento</title>
		...
		<div class="wrapper row-offcanvas row-offcanvas-left">
			...
			<aside class="right-side">
				<section class="content-header">
					<h1> Control post-procedimiento </h1>
					<ol class="breadcrumb">
						<li>
				...
				<section class="content">
					...
					<div class="row">
						<form action="controlprocedimiento.php" method="post" role="form" onsubmit="return validate()">
							<div class="col-xs-12">
								<div class="box">
									<?php
...
<b>Datos guardados!</b> El control post-procedimiento ha sido guardado exitosamente.
</div>';
										} else {
...
<b>Error!</b> Ocurri&oacute; un error al intentar guardar los datos, contacte a Soin Software (3007200405 - 4620915 en Bogot&aacute;).
									...
									<div class="box-header">
										<h3 class="box-title">Control post-procedimiento</h3>
									</div>
									<div class="box-body">
		...
		<div id="date-dialog" title="Error" style="display: none">
			<p>
				<span class="ui-icon ui-icon-cancel" style="float:left; margin:2px 7px 20px 0;"></span>La fecha del control no es valida.
			</p>
		</div>
